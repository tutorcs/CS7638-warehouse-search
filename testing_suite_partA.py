    ...
    def execute_student_plan(self, test_case, warehouse, boxes_todo):
        ...
        try:
            if TEST_MODE:
                ...
                warehouse_access[state.robot_position[0]][
                    state.robot_position[1]]  # mark robot starting position as visited

                student_planner = DeliveryPlanner_PartA(warehouse_access, state.robot_position, copy.deepcopy(boxes_todo), boxes) 
...
class PartATestCase(unittest.TestCase):
    ...
    def check_results(self, params):
        ...
        if not self.student_submission.viewed_cell_count.empty():
            ...
            viewed_cell_count = 99999

        if viewed_cell_count <= params['viewed_cell_count_threshold']:
            score = float(params['benchmark_cost']) / float(student_cost)
        else:
            score = 0
